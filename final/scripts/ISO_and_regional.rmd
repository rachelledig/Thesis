...
```{r}
# job::job({kuncondmeans_reg <- kfold(uncondmeans_reg)})
# 
# job::job({kuncondgrowth_reg <- kfold(uncondgrowth_reg)})
# job::job({kcomposite_reg <- kfold(composite_reg)})
job::job({kuncondmeans_ISO <- kfold(uncondmeans_ISO)
saveRDS(kuncondmeans_ISO, file = paste0(folder,"condeff_mu_composite_ISO.Rds"))})

job::job({kuncondgrowth_ISO <- kfold(uncondgrowth_ISO)
saveRDS(kuncondgrowth_ISO, file = paste0(folder,"condeff_mu_composite_ISO.Rds"))})

job::job({kcomposite_ISO <- kfold(composite_ISO, inits = "0")
saveRDS(kcomposite_ISO, file = paste0(folder,"condeff_mu_composite_ISO.Rds"))})
