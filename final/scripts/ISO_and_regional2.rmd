...
get_prior(data = data,
    bf(irrfrac ~ 0 + 
         ...
         zi ~ 0 + 
           ...
           popdens + 
           DD.regime + 
           (0 + Intercept + rugged + precip + dist + gdppc + popdens + DD.regime | eight_regions)),
    family = zero_inflated_beta(link = "logit", 
                                  link_phi = "identity", 
                                  link_zi = "logit"))

job::job({
composite2_reg <- brm(data = data,
    bf(irrfrac ~ 0 + 
         Intercept + 
         yearcount + 
         precip:yearcount + 
    ...
    backend = "cmdstanr",
    seed = seed,
    file = paste0(folder,"composite2_reg.Rds")
    )
  }) 
```
...
cond_composite2_reg_mu <- conditional_effects(composite2_reg, 
                                             conditions = conds_reg,
                                             re_formula = NULL,
                                             dpar = "mu",
                                             plot = FALSE)
saveRDS(cond_composite2_reg_mu, file = paste0(folder,"cond_composite2_reg_mu.Rds"))
```
...
get_prior(data = data,
    bf(irrfrac ~ 0 + 
         ...
         zi ~ 0 + 
           ...
           popdens + 
           DD.regime + 
           (0 + Intercept + rugged + precip + dist + gdppc + popdens + DD.regime | ISO)),
    family = zero_inflated_beta(link = "logit", 
                                  link_phi = "identity", 
                                  link_zi = "logit"))

job::job({
composite2_ISO <- brm(data = data,
    bf(irrfrac ~ 0 + 
         Intercept + 
         yearcount + 
         precip:yearcount + 
    ...
    backend = "cmdstanr",
    seed = seed,
    file = paste0(folder,"composite2_ISO.Rds")
    )
  }) 
```
...
cond_composite_ISO_zi <- conditional_effects(composite2_ISO, 
                                             conditions = conds_ISO,
                                             re_formula = NULL,
                                             dpar = "zi",
                                             plot = FALSE)
saveRDS(cond_composite_ISO_zi, file = paste0(folder,"cond_composite2_ISO_zi.Rds"))

cond_composite2_ISO_mu <- conditional_effects(composite2_ISO, 
                                             conditions = conds_ISO,
                                             re_formula = NULL,
                                             dpar = "mu",
                                             plot = FALSE)
saveRDS(cond_composite2_ISO_mu, file = paste0(folder,"cond_composite2_ISO_mu.Rds"))


########################
cond_notimeserieszi_zi <- conditional_effects(`165aa_5yronlyISO_notimeserieszi`, 
                                             conditions = conds_ISO,
                                             re_formula = NULL,
                                             dpar = "zi",
                                             plot = FALSE)
saveRDS(cond_notimeserieszi_zi, file = paste0(folder,"cond_notimeserieszi_zi.Rds"))



cond_notimeserieszi_mu <- conditional_effects(`165aa_5yronlyISO_notimeserieszi`, 
                                             conditions = conds_ISO,
                                             re_formula = NULL,
                                             dpar = "mu",
                                             plot = FALSE)
saveRDS(cond_notimeserieszi_mu, file = paste0(folder,"cond_notimeserieszi_mu.Rds"))
```
...
job::job({
  uncondmeans_ISO <- recompile_model(uncondmeans_ISO)
  kuncondmeans_ISO <- kfold(uncondmeans_ISO, inits = "0")
saveRDS(kuncondmeans_ISO, file = paste0(folder,"kuncondmeans_ISO.Rds"))})

job::job({
  uncondgrowth_ISO <- recompile_model(uncondgrowth_ISO)
  kuncondgrowth_ISO <- kfold(uncondgrowth_ISO, inits = "0")
saveRDS(kuncondgrowth_ISO, file = paste0(folder,"kuncondgrowth_ISO.Rds"))})

job::job({
  composite2_ISO <- recompile_model(composite2_ISO)
  kcomposite2_ISO <- kfold(composite2_ISO, inits = "0")
saveRDS(kcomposite2_ISO, file = paste0(folder,"kcomposite2_ISO.Rds"))})
