...
#combined
coefs <- as.data.frame(coef(`165aa_5yronlyISO_notimeserieszi`, groups="ISO", probs = 0.5))%>% exp()
...
pdf("/Volumes/RachelExternal2/Thesis/BayesMultilevelMods/figure/longplot.png", height = 20)
  `165aa_5yronlyISO_notimeserieszi` %>%
  spread_draws(b_Intercept, r_ISO[ISO,]) %>%
  # add the grand mean to the group-specific deviations
  mutate(mu = b_Intercept + r_ISO) %>%
  mutate(mu = exp(mu)) %>%
  ungroup() %>% 
  left_join(., Countrynames, by = "ISO") %>% 
  ggplot(aes(x = mu, y = reorder(Countryname, mu))) +
  geom_vline(xintercept = exp(fixef(`165aa_5yronlyISO_notimeserieszi`)[1, 1]), color = "#839496", size = 1) +
  geom_vline(xintercept = exp(fixef(`165aa_5yronlyISO_notimeserieszi`)[1, 3:4]), color = "#839496", linetype = 2) +
