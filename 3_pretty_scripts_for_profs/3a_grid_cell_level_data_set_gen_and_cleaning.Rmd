...
```{r rice_and_maize}
ricemaize = apply(cftfrac[,c(2,3,18,19),],c(1,3),sum) #summing rainfed and irrigated rice and maize 
ricemaize <- as.data.frame(ricemaize)
colnames(ricemaize) <- seq(1901, 2005, by = 1)
ricemaize$id <- seq(1, 67420, by=1)

ricemaize <- ricemaize %>% 
  pivot_longer(-id, names_to = "years", values_to = "rice_maize")

aei <- aei %>% 
  right_join(., ricemaize, by = c("years", "id"))
```
...
write.csv(aei_nocrop, "/Volumes/RachelExternal/Thesis/Data_and_Plots/aei_nocrop.csv")
```
