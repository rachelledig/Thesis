...
proj4string(nocropids) <- CRS("+proj=longlat +datum=WGS84") 
#converting to UTM (x, y) projection
res <- spTransform(nocropids, CRS("+proj=utm +ellps=WGS84 +datum=WGS84 +units=km" )) 

#sampling with a min dist of 3 cells between @equator 0.5deg cell is 55km
#3 cells = 165 km of min dist.
sub.nocropids <- subsample.distance(res, size = 2185, d = 165, echo = F)  
...
sub.nocropids2 <- spTransform(sub.nocropids, CRS("+proj=longlat +ellps=WGS84 +datum=WGS84" ))

world <- map_data("world") # we already did this, but we can do it again

ggplot() + geom_polygon(data = world, aes(x=long, y = lat, group = group), fill = NA, color = "black") + 
  geom_point(data = as.data.frame(nocropids), aes(x=lon, y=lat), size=3,shape=".", color = "red") + 
  geom_point(data = as.data.frame(sub.nocropids2), aes(x=lon, y=lat), size=3,shape=".", color = "blue") 

ggsave(plot = last_plot(), dpi = 300, width = 10, height = 6.6, filename = "/Volumes/RachelExternal/Thesis/Data_and_Plots/Plots/sampled_ids.png")
