...
write_csv(aei_small, "/Volumes/RachelExternal/Thesis/Data_and_Plots/aei_small.csv")
...
```{r histdist}
hist(aei_small$irrfrac, breaks = 50)
```


```{r histdist}
hist(aei_small$dist, breaks = 1000)
plot(aei_small$dist, aei_small$irrfrac)
```

```{r histmedinc}
hist(aei_small$medinc, breaks = 100000, xlim = c(-1, 1))
plot(aei_small$medinc, aei_small$irrfrac)
```

```{r histpopdens}
hist(aei_small$popdens, breaks = 100)
plot(aei_small$popdens, aei_small$irrfrac)
```

```{r histgdp}
hist(aei_small$gdppc, breaks = 100)
plot(aei_small$gdppc, aei_small$irrfrac)
```

```{r histprecip}
hist(aei_small$precip, breaks = 100)
plot(aei_small$precip, aei_small$irrfrac)
```

```{r histpet}
hist(aei_small$pet, breaks = 100)
plot(aei_small$pet, aei_small$irrfrac)
```

```{r}
hist(aei_small$humid, breaks = 100)
plot(aei_small$humid, aei_small$irrfrac)
```


```{r histrugg}
hist(aei_small$rugged, breaks = 100)
plot(aei_small$rugged, aei_small$irrfrac)
```
...
```{r}
pairs(~irrfrac+log(gdppc)+log(popdens)+rugged,data=aei_small) 
```
...
```{r}
pairs(~irrfrac + dist + medinc + Democracy + DD.regime,data=aei_small) 
```
...
```{r}
aei_small %>% select(precip, humid, popdens, gdppc, dist, medinc, rugged) %>% summary()
```
...
```{r}
minnz(aei_small$dist)
minnz(aei_small$popdens)
minnz(aei_small$precip)
minnz(aei_small$humid)
```
...
write.csv(aei_small_std, "/Volumes/RachelExternal/Thesis/Data_and_Plots/aei_small_std.csv")
