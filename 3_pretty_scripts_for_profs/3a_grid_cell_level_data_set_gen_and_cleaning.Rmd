...
Some variables need to be logged before centering on 0.
```{r}
aei_nocrop_samp %>% dplyr::select(precip, humid, popdens, gdppc, dist, medinc, rugged) %>% summary()
```
...
```{r}
minnz(aei_nocrop_samp$dist)
minnz(aei_nocrop_samp$popdens)
minnz(aei_nocrop_samp$precip)
minnz(aei_nocrop_samp$humid)
```
...
aei_nocrop_samp %>% select(precip, humid, popdens, gdppc, dist, medinc, rugged) %>% summary()
...
summary(aei_nocrop_samp_std)

write.csv(aei_nocrop_samp_std, "/Volumes/RachelExternal/Thesis/Data_and_Plots/aei_nocrop_samp_std.csv")
