...
```{r morani}
grid <- aei_small %>% subset(yearcount == 45) %>% select(id, lat, lon, irrfrac) %>% unique() #of the year 2005
write.csv(grid, "/Volumes/RachelExternal/Thesis/Data_and_Plots/latlonirrfrac.csv")
...
grid <- read.table("/Volumes/RachelExternal/Thesis/Data_and_Plots/latlonirrfrac.csv", sep=",", header=T)
moranfast(grid$irrfrac, grid$lon, grid$lat)
...
bubble(grid, zcol='irrfrac', fill=TRUE, do.sqrt=FALSE, maxsize=3)


TheVariogram=variogram(irrfrac~1, data=grid)
plot(TheVariogram)
```
...
```{r sampling}
samplegrid <- aei_small %>% select(lat, lon, id) %>% unique()
samplegrid <- reshape(samplegrid, idvar = "lat", timevar = "lon", v.names = "id", direction = "wide")
samplegrid <- samplegrid[order(-samplegrid$lat),]
