...
cl_ha <- NULL
cl_ha <- aggregateLPJmLdata2Country(input=cellarea/10000 , #ha conversion
...
syspeace <-
  read.csv2("/Volumes/RachelExternal/Thesis/Data_and_Plots/Country_Level_Data/Polity_center_for_systemic_peace.csv")
syspeace$polity2[is.na(syspeace$polity2)] <- "X"

#fixing the codes
library(countrycode)
aei <- syspeace %>% 
  mutate(ISO = countrycode(ccode, origin = 'cown', destination = 'iso3c'),
         yearcount = year - 1900) %>% 
