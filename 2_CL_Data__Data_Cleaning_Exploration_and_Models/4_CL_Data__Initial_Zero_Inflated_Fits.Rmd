...
```{r The Model, echo=TRUE, results='hide'}
#and the model
zi_simple_preds_1 <- 
  brm(
...
Ok, so divergent transitions are throught the roof. Lets knock out the $\phi$ parameter, cause I wanna model what is most nessary at this point. haha. 
...
zi_simple_preds_4_bf <- 
  bf(
  irrfrac ~ yearcount + (yearcount|six_regions),
  phi ~ 1, 
  zi ~ area_km,
  family = zero_inflated_beta() 
)
