...
aei_std <-
  ...
  mutate(across(c(8, 17:19, 20:23, 25:51), centered)) %>% 
  mutate(across(c(24), normalized))

summary(aei_std)
  
#write.csv(aei_std, "/Volumes/RachelExternal/Thesis/Data_upload_for_CL/AEI_Std.csv")
```
...
tr_rain_raw <-
  aei_no_std %>% 
  drop_na(Temprate_roots) %>% 
  ggplot(aes(x=Temprate_roots, color=six_regions)) +
  geom_histogram(fill="white", position="dodge", bins = 100) + 
  scale_color_brewer(palette="Dark2") + ggtitle("Temprate Roots - Rainfed")
tr_irr_raw <-
  aei_no_std %>% 
  drop_na(Temprate_roots.1) %>% 
  ggplot(aes(x=Temprate_roots.1, color=six_regions)) +
  geom_histogram(fill="white", position="dodge", bins = 100) + 
...
grid.arrange(trr_rain_raw, trr_rain_std, nrow = 4)
```
