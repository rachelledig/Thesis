...
aei2<- 
  ...
  mutate(area_km_std = scale(area_km), area_km_sc = area_km/max(area_km))
...
income <- income %>% 
  mutate(log_income_std = scale(log_income))
...
population <- population %>% 
  mutate(log_pop_std = scale(log_pop))
...
```{r}
aei2 <- merge(aei, iso, by = c("ISO","country", "year"), all.x = TRUE)
rm(iso)
summary(aei) 
```
...
aei2 <-
  ...
  na.omit() %>% 
  mutate(log_gdp_tot_std = scale(log_gdp_tot)) %>% 
...
precip <- 
  hum_PET_prec %>% 
  select(ISO, year, cubM_precip) %>% 
  mutate(precip_sc = cubM_precip/max(cubM_precip)) %>% 
  mutate(precip_std = cubM_precip/mean(cubM_precip)) %>% 
  mutate(log_precip = log(cubM_precip)) %>% #1 is added here to prevent -Inf
...
```{r Precip Hists}
hist(aei$cubM_precip, breaks = 100)
