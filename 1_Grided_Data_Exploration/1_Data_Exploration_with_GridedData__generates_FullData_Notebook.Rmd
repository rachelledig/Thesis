...
iFol="/Volumes/RachelExternal/Thesis/Thesis/Original_Grid_Data_Scripts/data/"
...
ggplot(globalIrrigArea) + geom_line(aes(year, globalIrrigArea, group = 1)) + xlab("Year") + ylab("Irrigated Area (Mha)") 
```
...
#lets add the regions of all of these countries
#NOTE TO RACHEL, WHERE DID THIS COME FROM?
regions <- read.csv(file.choose())
regions <- regions[, c(3,6)]
colnames(regions) <- c("ISO", "Region")
d <- merge(d, regions, on = "ISO", all.x = TRUE)
d$Region <- as.factor(d$Region)
#we have an issue with kosovo here and No_Land. They dont have assigned regions.
View(subset(d, is.na(d$Region)))
View(subset(d, is.na(d$Region)) %>% group_by(countryname) %>% count())
#we cant remove them until the end because it throws off the order of the cells. 
...
precipdf <- dimSums(prec_landuse, dim = 2) #we wanna sum over the months (dimension 2) so that precip is in (mm/year)
...
#now time to fix the NAs from Regions
#no_land has no data, effectively. no irrigation fraction or cft. i will remove it. Should I remove it?
#summary(subset(d, countryname== "No_Land") )
#d<-d[!(d$countryname=="No_Land"),]


# #Regardless, lets fix Kosovo, its in europe!
# d$Region[d$countryname == "Kosovo"]  <- "Europe" 
# d %>% subset(countryname=="Kosovo")

summary(d)
str(d)
#write_csv(d, "/Volumes/RachelExternal/Thesis/Thesis/DataFull.csv")
```
